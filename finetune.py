...
def run_model(args, device, train_loader, val_loader, post_transform, post_label, post_pred):
    ...
    train_rl_loader = preprocess_finetune.get_loaders(args.modality, args.batch_size, device, args.patch, True, args.lowres)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if train_rl_iter is None:
            train_rl_iter = iter(train_rl_loader)
...
def set_up():
    ...
    parser.add_argument("--weights", type=str, help="Path to pre-trained model to use for finetuning.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--vae", type=str, help="Path to VAE model for lesion generation. If empty will paste from lesion files."
    )
    ...
    parser.add_argument("--twod", default=False, action="store_true", help="Train in 2D. Useful for quick debugging.")
    parser.add_argument("--modality", type=str, help="Choice of modality to use for fine-tuning.")
    ...
    train_loader, val_loader, post_transform, post_label, post_pred = get_loaders(args.batch_size, args.vae, args.fshealthy, args.mbhealthy, 
                                                                                      args.twod, device, args.fade, args.debug, args.lowres, args.patch, True)

    return args, device, train_loader, val_loader, post_transform, post_label, post_pred


def main():
    args, device, train_loader, val_loader, post_transform, post_label, post_pred = set_up()
    if args.debug:
        ...
        exit()
    run_model(args, device, train_loader, val_loader, post_transform, post_label, post_pred)
